...
def Ln_Prior(theta):
    ...
    # O_k between -1 and 1
    if len(theta) == 7:             # If curvature = True
        if not -1 < theta[-1] < 1:
            return - np.inf
...
def Ln_Likelihood(theta, model, curvature, lenses, SHOES, TRGB):
    ...
    if curvature:
        M, r_s, c0, c1, c2, c3, O_k = theta
    else:
        M, r_s, c0, c1, c2, c3 = theta
        O_k = 0.0
    ...
    # Model 1
    if model == 1:
        expansion_H = c0 + c1 * z_total + c2 * z_total ** 2 + c3 * z_total ** 3
    ...
    # Model 2
        ...
        expansion_Dl = c0 * Z + c1 * Z ** 2 + c2 * Z ** 3 + c3 * Z ** 4
    ...
    # Model 3
        ...
        Dm_dz = (c0 + 2 * c1 * Z + 3 * c2 * Z ** 2 + 4 * c3 * Z ** 3) * Z_dz
        expansion_Dm = c0 * Z + c1 * Z ** 2 + c2 * Z ** 3 + c3 * Z ** 4
        expansion_Dl = expansion_Dm * (1 + z_total)
        c0 = c / c0
        expansion_H = c / Dm_dz * (1 + (c0 / c) ** 2 * O_k * expansion_Dm ** 2) ** 0.5
