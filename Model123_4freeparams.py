...
from Posterior_model123_4freeparams import *
...
class MCMC_model123_4fp:
    ...
    def __init__(self, model, curvature=False, lenses=False, SHOES=False, TRGB=False, save=True, testing=True,
        ...
        # Number of free parameters = nparam (4) + 1 (M from SN) + 1 (Rs from BAO)
        self.ndim = 6
    ...
    def get_params(self):
        ...
        self.c3 = ufloat(np.mean(self.flatchain[:, 5]), np.std(self.flatchain[:, 5]))
        theta.append(self.c0.n)
        theta.append(self.c1.n)
        theta.append(self.c2.n)
        theta.append(self.c3.n)
    ...
    def test_expansion(self):
        ...
        if self.model == 1:
            ...
            def H_func(z_, c0, c1, c2, c3):
                return c0 + c1 * z_ + c2 * z_ ** 2 + c3 * z_ ** 3

            print("Total n = ", nsamples)
            for n in range(nsamples):
                # Calculate distances and Hubble parameter
                H = H_func(z_list, self.flatchain[n, 2], self.flatchain[n, 3], self.flatchain[n, 4], self.flatchain[n, 5])
        ...
        # ------------------------------------------------------------------------------------------------
            ...
            def Dl_func(z_, c0, c1, c2, c3):
                return c0 * z_ + c1 * z_ ** 2 + c2 * z_ ** 3 + c3 * z_ ** 4
        ...
        # Insert Nan if std is showing unreliable fluctuations
        for h in range(1, len(H_std)):
            if abs(H_std[h] - H_std[h-1]) > 10 or np.isnan(H_std[h-1]):
                H_std[h] = np.nan
        ...
        # Save files
        with open("Testing/4fp_H_Dm_mu_params:_lenses=" + str(self.lenses) + "_SH0ES=" + str(self.SHOES) + "_TRGB=" +
        ...
        with open("Testing/4fp_Lenses_params:_lenses=" + str(self.lenses) + "_SH0ES=" + str(self.SHOES) + "_TRGB=" +
    ...
    def output_file(self):
        ...
        print("Model = " + str(self.model), file=open("Output/4fp_SN+BAO_lenses=" + str(self.lenses) + "_SH0ES=" + str(self.SHOES)
            + "_TRGB=" + str(self.TRGB) + "_model" + str(self.model) + "_curvature=" + str(self.curvature) + ".txt", "w"))

        # Define the file to write output to
        f = open("Output/4fp_SN+BAO_lenses=" + str(self.lenses) + "_SH0ES=" + str(self.SHOES) + "_TRGB=" + str(self.TRGB) +
        ...
        print("Free parameters: 4", file=f)
    ...
    def run(self):
        ...
        if self.model == 1:
            ...
            param_init4 = np.random.uniform(0, 100, self.nwalkers)
        ...
        # For expansion in distance: need larger starting values for c0, c1 and c2.
            ...
            param_init4 = np.random.uniform(20000, 40000, self.nwalkers)
            ...
            param_init4 = np.random.uniform(2000, 2500, self.nwalkers)

        # Define starting positions
        if self.curvature:
            p0 = np.array([M_init, rs_init, param_init1, param_init2, param_init3, param_init4, Ok_init]).T
        else:
            p0 = np.array([M_init, rs_init, param_init1, param_init2, param_init3, param_init4]).T
        ...
        Theta, Results = MCMC_model123_4fp.get_params(self)
        ...
        if self.testing:
            MCMC_model123_4fp.test_expansion(self)

        # Save the results
        if self.save:
            MCMC_model123_4fp.output_file(self)
            np.savetxt("Chains/4fp_SN+BAO_lenses=" + str(self.lenses) + "_SH0ES=" + str(self.SHOES) + "_TRGB=" +
