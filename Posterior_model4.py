...
def Ln_Prior(theta):
    """
    :param theta: Array with the guesses for the free parameters.
    ...
    # O_m between 0.05 and 0.5
    if not 0.05 < theta[3] < 0.5:
        return - np.inf

    # O_k between -1 and 1
    if len(theta) == 5:             # If curvature = True
        if not -1 < theta[-1] < 1:
            return - np.inf
...
def Ln_Likelihood(theta, curvature, lenses, SHOES, TRGB):
    """
    ...
    # Obtain H and Dm
    expansion_H = c0 * (Om * (1 + z_total)**3 + (1 - Om) + O_k * (1 + z_total)**2 )**0.5
...
def Ln_Posterior(theta, curvature, lenses, SHOES, TRGB):
    """
    ...
    """
    return Ln_Prior(theta) + Ln_Likelihood(theta, curvature, lenses, SHOES, TRGB)
